...
void ACTSSeededCKFTrackingProc::processEvent(LCEvent *evt)
{
  ...
  for (const std::string &collection : _inputTrackerHitCollections)
  {
    ...
    if (trackerHitCollection == nullptr)
      continue;
  ...
  std::shared_ptr<Acts::BinFinder<ACTSTracking::SeedSpacePoint>> bottomBinFinder = std::make_shared<Acts::BinFinder<ACTSTracking::SeedSpacePoint>>();
  std::shared_ptr<Acts::BinFinder<ACTSTracking::SeedSpacePoint>> topBinFinder = std::make_shared<Acts::BinFinder<ACTSTracking::SeedSpacePoint>>();

  std::unique_ptr<Acts::SpacePointGrid<ACTSTracking::SeedSpacePoint>> grid = Acts::SpacePointGridCreator::createGrid<ACTSTracking::SeedSpacePoint>(gridCfg);
