...
TrackTruthProc::TrackTruthProc()
    : Processor("TrackTruthProc")
{
...
void TrackTruthProc::processEvent( LCEvent * evt )
{
  ...
  for(int itT=0;itT<nTracks;++itT)
  {
    ...
    {
      float frac=static_cast<float>(mchit.second)/track->getTrackerHits().size();

      bool better = mcBestMatch_track.count(mchit.first)==0 || // no best match exists
  ...
  for(const std::pair<EVENT::MCParticle*,EVENT::Track*> &mctrk : mcBestMatch_track)
  {
    relMC2T.addRelation(mctrk.first, mctrk.second, mcBestMatch_frac[mctrk.first]);
  }
